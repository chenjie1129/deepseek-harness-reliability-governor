...
The bundled browser face renders only two bounded, fixed-purpose A2UI v0.9.1 Basic-catalog surfaces: interpreted intent and evidence contract. Proposal strings are treated as display text, and the selector rejects unknown component types, purpose-mismatched action names, question/option shapes, surface IDs, and proposal receipts. Actions return a structured answer to Harness; they cannot directly execute a business tool. A native Harness question remains the fallback. A2UI is not the approval authority, an execution sandbox, or a certification oracle.
...
Approved intent, contracts, and results are persisted in the session log, and both proposals are displayed to the reviewing client. Do not put secrets, tokens, private keys, passwords, sensitive source, or private file literals in an objective, intent field, auxiliary-author `context`, check `text`, or `argumentsContain` field. Normal Harness tool events may retain `reliability_draft` and `reliability_begin` arguments, and auxiliary mode sends its objective/context payload to the configured model provider. Custom draft and denied-review events exclude raw auxiliary reasoning and free-text feedback; they retain bounded metadata and content receipts. An approved version 5 contract intentionally persists the normalized intent so future verification and audit can reconstruct what was approved. Code-verification events store output byte counts, truncation facts, and content receipts—not raw stdout/stderr. The subprocess service removes credential-shaped ambient variables unless a trusted caller explicitly forwards them; this plugin forwards none.
