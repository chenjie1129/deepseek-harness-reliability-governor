...
这是一个可选启用的 DeepSeek Harness 组合包：先让用户审查 Agent 对需求的理解，再单独审查证据合约，最后把“模型说做完了”改成“确定性证据检查通过了”。
...
| 两阶段 A2UI 审批边界测试 | 官方 A2UI v0.9.1 处理器可接受意图和证据两个固定界面；批准、修改、拒绝、篡改、回退、子 Agent 和无 Provider 路径均 fail closed | 精确意图与证据提案可在激活前分别要求 Harness UI 决策；不代表任一提案本身正确。 |
...
默认情况下，`reliability_begin` 会在激活前暂停两次。第一张固定 [A2UI v0.9.1](https://a2ui.org/) Basic Catalog 卡片显示 Agent 理解的目标、约束、假设、非目标和歧义；只有精确批准意图后，第二张卡片才显示 Claim、Check、作者来源、覆盖警告和修复次数。没有该 Renderer 的客户端使用 Harness 原生问答界面显示相同提案。任一阶段要求修改、拒绝、取消、动作过期或 UI Provider 不可用，都不会激活合约。v5 合约同时保存已批准意图，并把证据审批收据绑定到该意图。A2UI 只负责呈现；Harness 的 live-root 用户问答通道拥有选择权，确定性检查仍拥有认证权。详见[两阶段审查](docs/CONTRACT_REVIEW.md)。

v0.7 支持 `file_exists`、`file_absent`、`file_contains`、`file_not_contains`、`file_equals`、`json_equals`、`tool_succeeded`、`tool_not_called`、`code_verification_succeeded` 和 `no_tool_errors` 十种检查。
...
v0.7 在激活前把每条已声明的成功条件映射到检查，并按独立证据权威而不是检查数量计数：同一个文件上的两个检查只算一个证据源。需要人工判断、当前不支持或独立证据不足的 Claim 会返回 `review-required`；精确字面量、仅存在性和轨迹类检查会给出脆弱性警告。详见[合约覆盖说明](docs/CONTRACT_COVERAGE.md)。单独的意图审查会让语义假设变得可见，但仍无法自动发现所有漏写需求或证明 Claim 完整表达了用户意图。
...
代码 Profile 默认为空，因为不同仓库的测试门禁不同。请按 [可信代码验证](docs/CODE_VERIFICATION.md) 配置经过审查的测试、类型检查和构建命令。内置 Skill 负责教模型如何工作；真正拥有判定权的是固定的运行时 Profile，而不是 Skill。`contractReview.mode: required` 是交互默认值，现在同时要求意图审批和证据审批。无人值守评测或自动化必须显式设置 `mode: off`；此时生成未审查的 v3 合约，不能宣称经过意图或证据批准。默认只由当前任务模型起草两份提案，不增加第二次 Provider 调用；`auxiliary-model` 仍只是可选的证据合约作者。供应商凭据、Endpoint 和模型目录仍在 Harness Models 中配置，Governor 不提供自动 fallback。
...
- 两次 UI 批准只表示 live Harness 问答通道分别接受了精确意图和证据提案；它们不认证法律身份、不证明用户已经理解，也不代表任务已经通过。
- v0.7 不判断视觉质量、超出配置检查范围的开放式语义正确性、被遗漏的 Claim、缺少权威证据的远程状态，也不会猜测未知副作用是否成功；两阶段审查让误解可见，但不能保证用户一定发现误解。
- 含本插件自定义必要事件的会话，在恢复时应继续安装本组合包。Harness `0.1.2-alpha.1` 尚未提供下游事件注册服务，因此 v0.7 会在加载时把这些事件注册进其导出的进程级持久化目录；若未来该兼容入口不可用，插件会显式报错而不是静默产生无法恢复的会话。
