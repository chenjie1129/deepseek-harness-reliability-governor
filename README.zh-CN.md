...
v0.3 支持 `file_exists`、`file_absent`、`file_contains`、`file_not_contains`、`file_equals`、`json_equals`、`tool_succeeded`、`tool_not_called`、`code_verification_succeeded` 和 `no_tool_errors` 十种检查。
...
- v0.3 不判断视觉质量、超出配置检查范围的开放式语义正确性、缺少权威证据的远程状态，也不会猜测未知副作用是否成功。
