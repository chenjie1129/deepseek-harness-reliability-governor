...
2. `reliability_begin_code` proposes a contract containing every profile marked `required`. In interactive mode it must also include the interpreted intent; the user reviews that intent first and the evidence contract second. In default/manual authoring, the runtime adds required profiles before review. In auxiliary authoring, call `reliability_draft` with `contract_kind: code`; the runtime injects the same required profiles before calculating the single-use draft receipt, and `reliability_begin_code` accepts only that exact full claim/check set. The model cannot remove required profiles in either path.
